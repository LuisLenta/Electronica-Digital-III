...
/*********************************************************************//**
 * @brief         Configuration for ExtMatchOutputType of match register
 * @param[in]    TIMx Pointer to timer device, should be:
 *               - LPC_TIM0: TIMER0 peripheral
 *               - LPC_TIM1: TIMER1 peripheral
 *               - LPC_TIM2: TIMER2 peripheral
 ...
 * @param[in]   ExtMatchOutputType output for external match
 *                     -ExtMatchOutputType: Select output for external match
 *                         +     0:    Do nothing for external output pin if match
 *                         +   	 1:    Force external output pin to low if match
 *                         +     2:    Force external output pin to high if match
 ...
 * @return         None
 **********************************************************************/
void TIM_SetMatchExt(LPC_TIM_TypeDef *TIMx, uint8_t MatchChannel, TIM_EXTMATCH_OPT ext_match)
{

    CHECK_PARAM(PARAM_TIMx(TIMx));
    CHECK_PARAM(PARAM_TIM_EXTMATCH_OPT(ext_match));

    // match output type
	TIMx->EMR     &= ~TIM_EM_MASK(MatchChannel);
	TIMx->EMR   |= TIM_EM_SET(MatchChannel,ext_match);
}
/*********************************************************************//**
