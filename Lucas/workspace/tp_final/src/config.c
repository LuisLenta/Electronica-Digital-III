...
void CONFIG_gpdmaConfig(void)
{
	...
	GPDMACfg.SrcMemAddr = (uint32_t)TABLA_DAC;		// Source memory
	GPDMACfg.DstMemAddr = 0;						// Destination memory - don't care
	GPDMACfg.TransferSize = sizeof(TABLA_DAC);		// Transfer size
	...
	GPDMACfg.DstMemAddr = (uint32_t) &rx_buf;		// Destination memory
	GPDMACfg.TransferSize = sizeof(rx_buf);			// Transfer size
	GPDMACfg.TransferWidth = 0;						// Transfer width - don't care
...
//	Channel0_Err = 0;								// Reset Error counter
	...
	GPDMA_ChannelCmd(2, ENABLE);					// Disable GPDMA Channel 2 (ADC2M)
